...
def _scenario(n_agents=3, N=10, tf=20.0):
    ...
    bc = BoundaryConditions(
        x0=StateVector(r=np.zeros(3), v=np.zeros(3), eps=np.array([0., 0., 0., 1.]), omega=np.zeros(3)),
        xf=StateVector(r=np.array([2., 2., 2.]), v=np.zeros(3), eps=np.array([0.5, 0.5, 0.5, 0.5]), omega=np.zeros(3)),
        tf=tf,
    )
...
def test_detumble_reduces_translational_and_angular_motion():
    ...
    dt = 0.5
    x0 = StateVector(r=np.zeros(3), v=np.array([0.1, -0.05, 0.08]),
                     eps=np.array([0., 0., 0., 1.]), omega=np.array([0.02, -0.03, 0.05]))
