...
from ..data_structures import StateVectorLie, Trajectory
from ..new_opts import so3_exp
from ..evaluation.metrics import lie_attitude_violation
...
class AgentController:
    ...
    # --------------------------------------------------------------- sensing
    @staticmethod
    def sense(true_payload_state: StateVectorLie) -> StateVectorLie:
    ...
    def _detumble_command(self, sensed: StateVectorLie) -> np.ndarray:
        ...
        """
        c = self.cfg
        R = so3_exp(np.asarray(sensed.phi, dtype=float).reshape(3))
    ...
    def update_detumble(self, sensed: StateVectorLie) -> bool:
    ...
    def _ref_state(self) -> StateVectorLie | None:
        if self.ref is None:
            return None
